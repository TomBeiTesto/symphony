...
Status: Draft v1 (language-agnostic, platform-agnostic)
...
(GitLab, Linear, or a built-in local board), creates an isolated workspace for each issue, and runs a
...
- Multi-tenant control plane or user authentication.
...
- A shell environment for hook and subprocess execution:
  - macOS/Linux: `bash` (or `sh`) available on `PATH`.
  - Windows: `cmd.exe` or `powershell.exe` available on `PATH` (alternatively `bash` via
    WSL or Git for Windows).
...
- `project_id` (string or null)
  - Links the issue to a project (local board only).
- `parent_issue_id` (string or null)
  - Links the issue to a parent issue for lineage tracking.
  - Set when a follow-up proposal is accepted and creates a new child issue.
- `propose_followups` (boolean)
  - Whether the agent should propose follow-up work after completing the task.
  - Default: `true`.
...
- `token_budget_exceeded` (boolean; set to true when the agent LLM reports token exhaustion; disables auto-polling)
...
- `terminal_states` (list of strings or comma-separated string)
  - Default: `Closed`, `Cancelled`, `Canceled`, `Duplicate`, `Done`, `Review`, `Archived`
...
- `root` (path string or `$VAR`)
  - Default: `<system-temp>/symphony_workspaces`
    - On macOS/Linux this is typically `/tmp/symphony_workspaces`.
    - On Windows this is typically `%TEMP%\symphony_workspaces`.
  - `~` and strings containing path separators (`/` or `\`) are expanded.
  - Bare strings without path separators are preserved as-is (relative roots are allowed but
    discouraged).
  - Implementations must normalize path separators to the host OS convention before use.

#### 5.3.4 `hooks` (object)

Fields:

- `after_create` (multiline script string, optional)
  - Runs only when a workspace directory is newly created.
  - Failure aborts workspace creation.
- `before_run` (multiline script string, optional)
  - Runs before each agent attempt after workspace preparation and before launching the coding
    agent.
  - Failure aborts the current attempt.
- `after_run` (multiline script string, optional)
  - Runs after each agent attempt (success, failure, timeout, or cancellation) once the workspace
    exists.
  - Failure is logged but ignored.
- `before_remove` (multiline script string, optional)
  - Runs before workspace deletion if the directory exists.
  - Failure is logged but ignored; cleanup still proceeds.
- `shell` (string, optional)
  - Override the shell used to execute hooks.
  - Default: auto-detected from host OS (see Section 9.4).
  - Examples: `bash`, `sh`, `pwsh`, `powershell`, `cmd`.
...
#### 5.3.6 `agent_process` (object)

Fields:

For agent-process-owned config values such as `approval_policy`, `thread_sandbox`, and
`turn_sandbox_policy`, supported values are defined by the targeted app-server version.
Implementors should treat them as pass-through agent process config values rather than relying on a
hand-maintained enum in this spec. To inspect the installed agent schema, run
`<agent-command> generate-json-schema --out <dir>` (if supported) and inspect the relevant
...
- `command` (string shell command)
  ...
  - The runtime launches this command via the host shell (see Section 10.1) in the workspace
    directory.
  - The launched process must speak a compatible app-server protocol over stdio.
- `shell` (string, optional)
  - Override the shell used to launch the agent process command.
  - Default: auto-detected from host OS (see Section 10.1).
  - Examples: `bash`, `sh`, `pwsh`, `cmd`.
- `approval_policy` (approval policy value)
  - Default: implementation-defined.
- `thread_sandbox` (sandbox mode value)
  - Default: implementation-defined.
- `turn_sandbox_policy` (sandbox policy value)
  - Default: implementation-defined.
...
Follow-up proposal instructions:

- When `issue.propose_followups` is `true` (default), the prompt renderer appends instructions
  asking the agent to propose follow-up work in a fenced `follow-ups` JSON block.
- The orchestrator extracts follow-up proposals from the agent's response and persists them in the
  agent run data.
- Issues with pending follow-ups are moved to the "Review" state instead of "Done".
- Follow-ups can be accepted (creating a new child issue with `parent_issue_id`) or rejected via
  the board UI.
- When all follow-ups are resolved, the issue auto-moves from "Review" to "Done".
...
- Path/command fields support:
  - `~` home expansion (maps to `$HOME` on POSIX, `%USERPROFILE%` on Windows)
  - `$VAR` expansion for env-backed path values (`$VAR` syntax on all platforms;
    implementations should also accept `%VAR%` on Windows for path values)
  - Apply expansion only to values intended to be local filesystem paths; do not rewrite URIs or
    arbitrary shell command strings.
  - Normalize path separators to the host OS convention (`/` on POSIX, `\` on Windows) after
    expansion.
...
- The software should attempt to adjust live behavior to the new config (for example polling
  cadence, concurrency limits, active/terminal states, agent process settings, workspace paths/hooks, and
...
- `agent_process.command` is present and non-empty.
...
- `tracker.terminal_states`: list/string, default `Closed, Cancelled, Canceled, Duplicate, Done, Review, Archived`
...
- `agent_process.command`: shell command string, default `agent-server`
- `agent_process.approval_policy`: approval policy value, default implementation-defined
- `agent_process.thread_sandbox`: sandbox mode value, default implementation-defined
- `agent_process.turn_sandbox_policy`: sandbox policy value, default implementation-defined
- `agent_process.turn_timeout_ms`: integer, default `3600000`
- `agent_process.read_timeout_ms`: integer, default `5000`
- `agent_process.stall_timeout_ms`: integer, default `300000`
...
2. Auto-archive Done issues (move issues in "Done" state older than the archive threshold to
   "Archived"). Default threshold: 1 day.
...
- For each running issue, compute `elapsed_ms` since:
  ...
  - `started_at`
- If `elapsed_ms > agent_process.stall_timeout_ms`, terminate the worker and queue a retry.
...
### 8.6 Token Exhaustion Circuit Breaker

When an agent worker fails with an error that indicates LLM token quota exhaustion (rate limit,
429, quota exceeded, billing limit, etc.), the orchestrator:

1. Sets `token_budget_exceeded = true` on the runtime state.
2. Moves the failed issue and all other currently running issues to "Backlog" (local board only).
3. Logs a warning with the error details.

While `token_budget_exceeded` is `true`:

- The tick handler skips `run_tick` entirely (no polling, no dispatch, no reconciliation).
- The tick timer continues scheduling so the system can resume if the flag is reset.
- The dashboard displays a prominent banner indicating budget exhaustion.

This is a reactive mechanism — it detects exhaustion from the LLM's own error responses rather than
a pre-configured token cap. The `max_total_tokens` config field provides an optional proactive cap
...
- Execute in a local shell context appropriate to the host OS, with the workspace directory as
  `cwd`.
- Shell selection (in precedence order):
  1. Explicit `hooks.shell` config value, if set.
  2. Auto-detect from host OS:
     - macOS / Linux: `bash -lc <script>` (fallback: `sh -c <script>`).
     - Windows: `cmd /C <script>` (fallback: `powershell -NoProfile -Command <script>`).
- Implementations may allow `bash` on Windows when available (for example via WSL or Git for
  Windows) to maximize hook portability across platforms.
...
- Normalize both paths to absolute using OS-native resolution (this handles drive letters on
  Windows, symlinks on POSIX, UNC paths, etc.).
- Perform path comparison using the OS-native canonical form (case-insensitive on Windows,
  case-sensitive on macOS/Linux by default).
...
- Shell invocation (in precedence order):
  1. Explicit `agent_process.shell` config value, if set.
  2. Auto-detect from host OS:
     - macOS / Linux: `bash -lc <agent_process.command>`
     - Windows: `cmd /C <agent_process.command>` (or `powershell -NoProfile -Command
       <agent_process.command>` when `cmd` is insufficient)
- Implementations may allow `bash` on Windows when available (for example via WSL or Git for
  Windows) to maximize portability.

Notes:

- The default command is `agent-server`.
...
1. `initialize` request
   ...
   - If the targeted app-server requires capability negotiation for dynamic tools, include the
...
3. `thread/start` request
   - Params include:
     ...
     - If optional client-side tools are implemented, include their advertised tool specs using the
       protocol mechanism supported by the targeted app-server version.
...
- `agent_process.read_timeout_ms`: request/response timeout during startup and sync requests
- `agent_process.turn_timeout_ms`: total turn stream timeout
- `agent_process.stall_timeout_ms`: enforced by orchestrator based on event inactivity
...
- Run under a dedicated OS user (or a restricted Windows service account).
- Restrict workspace root permissions (POSIX `chmod`/`chown`; Windows ACLs).
- Mount workspace root on a dedicated volume if possible.
- On Windows, consider running inside a WSL2 distribution or container for additional filesystem
...
Workspace hooks are arbitrary scripts from `WORKFLOW.md`.

Implications:

- Hooks are fully trusted configuration.
- Hooks run inside the workspace directory using the configured or auto-detected host shell.
- Hook authors should be aware that hooks may execute under different shells on different
  platforms. Cross-platform workflows should use portable script syntax or provide
...
- `agent_process.command` is preserved as a shell command string
...
- Launch command uses workspace cwd and invokes the agent process command via the host shell
  (see Section 10.1 for per-OS shell selection)
- Startup handshake sends `initialize`, `initialized`, `thread/start`, `turn/start`
- `initialize` includes client identity/capabilities payload required by the targeted
  app-server protocol
...
- Agent process launch command config (`agent_process.command`, default `agent-server`)
...
- On Windows, verify that the selected shell (`cmd`, `powershell`, or `bash` via WSL/Git)
  correctly executes configured hooks and the agent process command.
- Verify workspace path normalization handles OS-specific conventions (drive letters on Windows,
  symlinks on POSIX).
