defmodule SymphonyElixir.PipelineSeedsTest do
  use SymphonyElixir.BoardCase, project_prefix: "PS"
  ...
  describe "PipelineSeed.seed/0" do
    ...
    test "extraction pipeline graph is fully connected" do
      ...
      start_ids =
        pipeline.nodes |> Enum.filter(&(&1.type == "start")) |> Enum.map(& &1.id) |> MapSet.new()
  ...
  describe "HardeningSeed.seed/0" do
    ...
    test "hardening pipeline has scan→gate→apply triplets" do
      ...
      for scan <- scan_nodes do
        ...
        assert Enum.any?(
                 pipeline.edges,
                 &(&1.source_node_id == scan.id && &1.target_node_id == gate_id)
               ),
               "Missing edge from #{scan.id} to #{gate_id}"

        assert Enum.any?(
                 pipeline.edges,
                 &(&1.source_node_id == gate_id && &1.target_node_id == apply_id)
               ),
  ...
  describe "FeaturePipelineSeed.seed/0" do
    ...
    test "feature pipeline has no orphaned nodes" do
      ...
      end_ids =
        pipeline.nodes |> Enum.filter(&(&1.type == "end")) |> Enum.map(& &1.id) |> MapSet.new()
  ...
  describe "UIPipelineSeed.seed/0" do
    test "creates the UI pipeline" do
      assert :ok = UIPipelineSeed.seed()

      pipelines = LocalBoard.list_pipelines()
      assert Enum.any?(pipelines, &(&1.name == "UI & Layout Design"))
    end

    test "UI pipeline has correct node count" do
      :ok = UIPipelineSeed.seed()

      {:ok, pipeline} =
        LocalBoard.list_pipelines()
        |> Enum.find(&(&1.name == "UI & Layout Design"))
        |> then(&LocalBoard.get_pipeline(&1.id))

      # start + 3 research + layout + plan-gate + 3 implement + code-gate +
      # 2 polish + quality-gate + kb-sync + end = 15
      assert length(pipeline.nodes) == 15
    end

    test "UI pipeline all edges reference valid nodes" do
      :ok = UIPipelineSeed.seed()
    ...
    test "UI pipeline has no orphaned nodes" do
      :ok = UIPipelineSeed.seed()
    ...
    test "UI pipeline phase 1 fans out from start" do
      :ok = UIPipelineSeed.seed()
      ...
      assert targets == ["a11y-audit", "design-audit", "ux-research"]
    end

    test "UI pipeline issue nodes have skill_ids populated" do
      :ok = UIPipelineSeed.seed()
    ...
    test "idempotent — running twice does not create duplicates" do
      ...
      count =
        LocalBoard.list_pipelines()
        |> Enum.count(&(&1.name == "UI & Layout Design"))
  ...
  describe "all seeds together" do
    test "all four pipelines coexist" do
      :ok = PipelineSeed.seed()
      :ok = HardeningSeed.seed()
      :ok = FeaturePipelineSeed.seed()
      :ok = UIPipelineSeed.seed()
