...
defmodule SymphonyElixir.SkillsSeed do
  ...
  @built_in_skills [
    ...
    %{
      ...
      "description" =>
        "SCAN: Detect lint and format violations across all subprojects. No code changes —" <>
    ...
    %{
      ...
      "description" =>
        "APPLY: Fix accepted lint and format violations. Only apply findings approved in the gate.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "SCAN: Find dead code, unused imports, unreachable branches across all subprojects." <>
    ...
    %{
      ...
      "description" =>
        "APPLY: Remove accepted dead code findings. Only remove items approved in the gate.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "SCAN: Audit dependencies for vulnerabilities, outdated versions, unused packages." <>
    ...
    %{
      ...
      "description" =>
        "APPLY: Fix accepted dependency findings. Only update/remove deps approved in the gate.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "SCAN: Find security vulnerabilities (OWASP top 10, secrets, injection) across all" <>
    ...
    %{
      ...
      "description" =>
        "APPLY: Fix accepted security findings. Only fix vulnerabilities approved in the gate.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "SCAN: Find duplicated code and patterns across all subprojects." <>
    ...
    %{
      ...
      "description" =>
        "APPLY: Extract and refactor accepted duplication findings. Only fix approved items.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "SCAN: Find error handling issues (bare rescues, swallowed errors) across all" <>
          " subprojects. No fixes — output findings.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "SCAN: Find missing type annotations and type errors across all subprojects." <>
          " No changes — output findings.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "APPLY: Add accepted type annotations and fix accepted type errors. Only apply approved items.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "SCAN: Find test style inconsistencies across all subprojects. No changes — output findings.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "SCAN: Find infrastructure issues (CI/CD, Docker, config, .gitignore) across all" <>
    ...
    %{
      ...
      "description" =>
        "SCAN: Find broken/missing Playwright E2E tests. Mark N/A if no frontend." <>
    ...
    %{
      ...
      "description" =>
        "APPLY: Fix/add accepted Playwright E2E tests. Only apply approved findings.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "SCAN: Measure test coverage, find gaps and duplicates across all subprojects." <>
          " No changes — output findings.",
      "category" => "hardening",
    ...
    %{
      ...
      "description" =>
        "APPLY: Add missing tests and remove duplicates for accepted findings." <>
    ...
    %{
      ...
      "description" => "Update documentation and changelog for the implemented feature.",
      "category" => "feature-implementation",
    ...
    # ── UI & Layout Design Pipeline Skills ──
    %{
      "name" => "ui-design-system-audit",
      "description" =>
        "Audit the existing design system: tokens, components, CSS architecture, inconsistencies.",
      "category" => "ui-pipeline",
      "tags" => "ui,design-system,audit,tokens,css",
      "built_in" => true,
      "content" => """
      # Design System Audit
      
      ## Goal
      Produce a comprehensive inventory of the existing design system.
      
      ## Process
      ### 1. Token Inventory
      - Map all color variables/tokens (backgrounds, text, accents, borders, status colors)
      - Map spacing scale (padding, margin, gap values actually used)
      - Map typography (font families, sizes, weights, line heights)
      - Map border radii, shadows, transitions, z-indices
      
      ### 2. Component Inventory
      - List every reusable UI component (buttons, cards, modals, forms, tables, badges, etc.)
      - For each: note its CSS approach, variants, and where it's used
      - Identify components that exist in multiple slightly-different versions
      
      ### 3. Inconsistency Report
      - Flag magic numbers (spacing/sizing not on the token scale)
      - Flag duplicate component implementations
      - Flag inconsistent naming conventions
      - Flag unused or orphaned CSS
      
      ### 4. CSS Architecture
      - Document the CSS methodology (BEM, utility classes, CSS-in-JS, etc.)
      - Note any framework usage (Tailwind, Bootstrap, custom)
      - Identify the layering/specificity strategy
      
      ## Output
      Write `/workspace/reports/DESIGN_SYSTEM_AUDIT.md` with all findings.
      
      ## Rules
      - Before running any commands, install project dependencies first
      - Do NOT make any changes — audit and report only
      - Include concrete code references (file:line) for every finding
      """
    },
    %{
      "name" => "ui-ux-research",
      "description" =>
        "Research UX patterns, layout hierarchy, and interaction paradigms for the feature area.",
      "category" => "ui-pipeline",
      "tags" => "ui,ux,research,patterns,layout",
      "built_in" => true,
      "content" => """
      # UX Research & Patterns
      
      ## Goal
      Research and document the best UX patterns for the target feature area.
      
      ## Process
      ### 1. Feature Analysis
      - Identify what user goals this UI serves
      - Map the information hierarchy: what's most important → least important
      - Determine the primary interaction mode (browse, search, create, configure, monitor)
      
      ### 2. Layout Patterns
      - Recommend layout pattern (dashboard grid, master-detail, wizard, feed, kanban, etc.)
      - Define visual scanning path (F-pattern for text-heavy, Z-pattern for sparse)
      - Plan information density: what to show immediately vs. progressive disclosure
      - Consider whitespace rhythm: tighter within groups, looser between groups
      
      ### 3. Interaction Paradigms
      - Inline editing vs. modal vs. page navigation — pick based on context weight
      - Optimistic updates vs. loading states — pick based on operation cost
      - Keyboard shortcuts for power users, obvious click targets for discovery
      - Smart defaults that reduce required input
      
      ### 4. Visual Hierarchy
      - Size, weight, and color to establish hierarchy (biggest/boldest = most important)
      ...
      - Use color sparingly and intentionally: status, emphasis, interaction affordance
      
      ## Output
      Write `/workspace/reports/UX_PATTERNS.md` with recommendations.
      
      ## Rules
      - Reference the design system audit findings when recommending patterns
      - Every recommendation must justify WHY it serves the user's goal
      - Do NOT propose patterns that contradict the existing design language unless explicitly asked
      """
    },
    %{
      "name" => "ui-accessibility-audit",
      "description" =>
        "Audit the UI for WCAG 2.1 AA compliance: contrast, keyboard nav, ARIA, semantics.",
      "category" => "ui-pipeline",
      "tags" => "ui,accessibility,a11y,wcag,aria",
      "built_in" => true,
      "content" => """
      # Accessibility Audit
      
      ## Goal
      Identify all accessibility violations and missing patterns in the current UI.
      
      ## Process
      ### 1. Semantic HTML
      - Verify heading hierarchy (h1 → h2 → h3, no skipped levels)
      - Check landmark regions (nav, main, aside, footer)
      - Verify lists use ul/ol/li, tables use proper thead/tbody/th
      - Check forms have associated labels (not just placeholder text)
      ...
      - Escape closes modals/dropdowns, Enter activates buttons
      - Skip-to-content link present
      
      ### 4. Screen Readers
      - ARIA labels on icon-only buttons and links
      - Live regions for dynamic content updates (aria-live)
      - Meaningful alt text on images, empty alt on decorative images
      ...
      ## Output
      Write `/workspace/reports/A11Y_AUDIT.md` with violations and recommendations.
      
      ## Rules
      - Before running any commands, install project dependencies first
      - Include WCAG criterion reference for each violation (e.g., "1.4.3 Contrast")
    ...
    %{
      ...
      "description" =>
        "Design the layout structure: grid system, breakpoints, component hierarchy, CSS plan.",
      "category" => "ui-pipeline",
      "tags" => "ui,layout,architecture,grid,responsive,css",
      "built_in" => true,
      "content" => """
      # Layout Architecture
      
      ## Goal
      Produce a complete layout blueprint from the research and audit outputs.
      
      ## Inputs
      Read and synthesize:
      - DESIGN_SYSTEM_AUDIT.md — existing tokens, components, CSS approach
      - UX_PATTERNS.md — recommended layout patterns, hierarchy, interactions
      - A11Y_AUDIT.md — accessibility requirements to build in from the start
      
      ## Process
      ### 1. Grid & Spacing System
      - Define or confirm the grid (12-column, CSS Grid areas, flexbox, etc.)
      - Define breakpoints with rationale (mobile: 320-767, tablet: 768-1023, desktop: 1024+)
      - Confirm the spacing scale (e.g., 4/8/12/16/24/32/48px)
      
      ### 2. Component Hierarchy
      - Draw the component tree: page → sections → containers → components → elements
      - Define each component's props/slots/variants
      - Identify shared vs. page-specific components
      - Plan component composition patterns (slot-based, render-prop, etc.)
      
      ### 3. Responsive Strategy
      - Document how each section reflows at each breakpoint
      - Sidebar → drawer on mobile, grid → stack, table → cards, etc.
      - Identify components that hide/show at breakpoints
      - Plan touch-friendly alternatives for hover-dependent interactions
      
      ### 4. CSS Architecture
      - Choose methodology that matches the project (extend existing, don't fight it)
      - Define naming conventions for new classes/tokens
      - Plan CSS custom properties for theming
      - Identify which existing components to reuse vs. create new
      
      ## Output
      Write `/workspace/reports/LAYOUT_ARCHITECTURE.md`.
      
      ## Rules
      - Every spacing value must come from the scale — no magic numbers
      - Every component must have defined responsive behavior
      - The plan must be implementable with the existing tech stack
      """
    },
    %{
      "name" => "ui-component-implementation",
      "description" =>
        "Build UI components from the layout architecture: semantic HTML, design tokens, composable APIs.",
      "category" => "ui-pipeline",
      "tags" => "ui,components,implementation,html,css",
      "built_in" => true,
      "content" => """
      # Component Implementation
      
      ## Goal
      Build all UI components following the approved layout architecture.
      
      ## Inputs
      Read LAYOUT_ARCHITECTURE.md for the component tree, props, variants, and CSS plan.
      
      ## Process
      ### 1. Structure First
      - Start with semantic HTML structure (no styling)
      - Ensure correct landmark regions, heading levels, form labels
      - Verify the DOM order matches the logical reading order
      
      ### 2. Apply Design Tokens
      - Use existing CSS variables/tokens for colors, spacing, typography
      - Create new tokens ONLY if the design system audit identified gaps
      - No hardcoded colors, magic number spacing, or inline styles
      
      ### 3. Component APIs
      - Each component is self-contained with clear props/attributes
      - Use consistent naming: actions (onClick), state (isOpen), content (title, children)
      - Provide sensible defaults; require only what's truly mandatory
      - Components should be composable — avoid mega-components
      
      ### 4. States
      - Every component must handle: default, hover, focus, active, disabled
      - Every data-driven component must handle: loading, empty, error, success
      - Use skeleton loaders for initial load, not spinners
      - Empty states should be helpful ("No items yet. Create one?")
      
      ## Rules
      - Before running any commands, install project dependencies first
      - Follow the project's existing CSS methodology
      - Do NOT add dependencies without explicit approval
      - Verify each component renders correctly before moving to the next
      """
    },
    %{
      "name" => "ui-responsive-mobile",
      "description" =>
        "Implement responsive behavior: mobile-first CSS, touch targets, fluid typography, breakpoints.",
      "category" => "ui-pipeline",
      "tags" => "ui,responsive,mobile,breakpoints,touch",
      "built_in" => true,
      "content" => """
      # Responsive & Mobile
      
      ## Goal
      Make every component and layout work beautifully across all viewport sizes.
      
      ## Process
      ### 1. Mobile-First CSS
      - Write base styles for mobile (320px+)
      - Add complexity with min-width media queries for larger viewports
      - Test at: 320px, 375px, 768px, 1024px, 1440px
      
      ### 2. Touch Targets
      - All interactive elements ≥ 44x44px on touch devices
      - Adequate spacing between adjacent tap targets (min 8px gap)
      - Touch-friendly alternatives for hover menus and tooltips
      
      ### 3. Fluid Typography & Spacing
      - Use clamp() or fluid scales where appropriate
      - Headings and body text should scale proportionally
      - Container max-widths prevent overly long line lengths (max ~75 chars)
      
      ### 4. Layout Reflow
      - Multi-column → single-column on small screens
      - Sidebars → bottom sheets or drawers on mobile
      - Tables → card lists or horizontal scroll on small screens
      - Navigation → hamburger/bottom-nav on mobile
      
      ### 5. Performance
      - Lazy load below-the-fold images and heavy components
      - Avoid layout shifts (set explicit dimensions on images/embeds)
      - Test with throttled CPU/network
      
      ## Rules
      - Before running any commands, install project dependencies first
      - Do NOT hide important content on mobile — restructure, don't remove
      - Test in actual browser at each breakpoint, not just CSS inspection
      """
    },
    %{
      "name" => "ui-interaction-motion",
      "description" =>
        "Add micro-interactions and motion design: transitions, animations, loading states.",
      "category" => "ui-pipeline",
      "tags" => "ui,animation,motion,transitions,micro-interactions",
      "built_in" => true,
      "content" => """
      # Interaction & Motion Design
      
      ## Goal
      Add purposeful motion that communicates state changes and guides attention.
      
      ## Principles
      - **Purposeful**: Every animation must communicate something (state change, causality, attention)
      - **Fast**: UI transitions 150-300ms. Anything longer feels sluggish.
      - **Consistent**: Same type of action → same type of motion everywhere
      - **Respectful**: Honor prefers-reduced-motion — provide instant alternatives
      
      ## Process
      ### 1. State Transitions
      - Hover: subtle scale (1.02) or background shift, 150ms ease
      - Focus: visible ring animation, 100ms
      - Active/pressed: slight scale-down (0.98), 50ms
      - Disabled: opacity reduction, no transitions needed
      
      ### 2. View Transitions
      - Content entering: fade-in + slight translate-up, 200ms ease-out
      - Content leaving: fade-out, 150ms ease-in
      - Page transitions: crossfade or shared-element morph
      - Stagger entrance of list items (30-50ms delay per item, max 300ms total)
      
      ### 3. Loading & Progress
      - Skeleton screens for initial loads (pulsing gradient)
      - Spinner only for indeterminate short operations (< 5s)
      ...
      - Toast/notification: slide-in from edge, auto-dismiss after 5s
      ...
      - Group animations with CSS custom properties for easy theming
      - Wrap all animation in `@media (prefers-reduced-motion: no-preference)`
      
      ## Rules
      - Before running any commands, install project dependencies first
      - No animation > 500ms for UI interactions
      - No bouncing, spinning, or gratuitous effects
      - Motion must feel native to the platform, not decorative
      """
    },
    %{
      "name" => "ui-visual-polish",
      "description" =>
        "Final visual pass: spacing, color, typography, dark/light themes, edge states.",
      "category" => "ui-pipeline",
      "tags" => "ui,polish,visual,theming,states",
      "built_in" => true,
      "content" => """
      # Visual Polish & Theming
      
      ## Goal
      Final visual refinement pass to make the UI feel crisp, intentional, and professional.
      
      ## Process
      ### 1. Spacing Audit
      - Verify every spacing value is on the scale (no 13px, 17px, 23px)
      - Consistent padding within same component types
      - Adequate breathing room — don't pack elements too tightly
      - Alignment: elements in the same row share the same baseline/center
      
      ### 2. Color Audit
      - Verify against the design system palette — no rogue hex values
      - Check status colors are used consistently (green=success, red=error, yellow=warning)
      - Ensure sufficient contrast on all text (use the a11y audit as reference)
      - Test with color blindness simulators (protanopia, deuteranopia)
      
      ### 3. Typography Audit
      - Heading hierarchy is visually clear (h1 obviously larger than h2)
      - Body text is readable (14-16px, 1.5 line-height for paragraphs)
      - Monospace for code, tabular numbers for data columns
      - Text truncation with ellipsis + tooltip where needed
      
      ### 4. Edge States
      - Empty states: helpful illustration or message + primary action
      - Error states: clear message + recovery action
      - Loading states: skeletons in the right shape, not just spinners
      - Long content: truncation, scrollable areas, "show more" patterns
      - Overflow: test with very long strings, many items, zero items
      
      ### 5. Theme Support
      - If dark/light theme exists, verify all new components work in both
      - Use CSS custom properties, not hardcoded colors
      - Test switching between themes without page reload
      
      ## Rules
      - Before running any commands, install project dependencies first
      - Only change visual presentation — no functional changes
      - Every change must use existing design tokens
      """
    },
    %{
      "name" => "ui-accessibility-compliance",
      "description" =>
        "Fix a11y violations and verify new components: ARIA, keyboard nav, contrast, screen readers.",
      "category" => "ui-pipeline",
      "tags" => "ui,accessibility,a11y,compliance,fix",
      "built_in" => true,
      "content" => """
      # Accessibility Compliance
      
      ## Goal
      Fix all accessibility violations and ensure new components are fully accessible.
      
      ## Inputs
      Read A11Y_AUDIT.md for the list of violations to fix.
      
      ## Process
      ### 1. Fix Audit Violations
      - Work through each violation from the audit
      - Fix in order: critical (blocks usage) → serious (degrades experience) → minor
      
      ### 2. Verify New Components
      - Every new interactive element has a visible focus indicator
      - Every new button/link has an accessible name (text content or aria-label)
      - Every new form input has an associated label
      - Every new image has appropriate alt text
      - Every new modal traps focus and returns focus on close
      
      ### 3. Keyboard Testing
      - Tab through the entire page — everything reachable, logical order
      - Enter/Space activates buttons and links
      - Arrow keys navigate within composite widgets (tabs, menus, radio groups)
      - Escape closes any overlay
      
      ### 4. Screen Reader Testing
      - Read through the page — does the content make sense linearly?
      - Dynamic updates announced via aria-live regions
      - Form errors announced when they appear
      
      ### 5. Automated Checks
      - Run axe-core or equivalent automated checker
      - Fix any new violations
      - Document remaining known issues with justification
      
      ## Rules
      - Before running any commands, install project dependencies first
      - Do NOT remove functionality to "fix" accessibility — enhance it
      - Every fix must include the WCAG criterion it addresses
  ...
  @default_groups [
    ...
    %{
      "name" => "UI & Layout Design Pipeline",
      "description" =>
        "Skills for the UI & Layout Design pipeline — audit, research, architect, implement, polish",
      "skill_names" => [
